...
- **Sí hay columna lateral de categorías**, como en ML. El riel es markup nuestro (`.ma-sheet__rail`) y se arma leyendo los `.filter-block-label`; mostrar un grupo por vez es `display` por `:nth-child(N)` sobre los hijos de `#FilterContent`, que son todos `.filter-block` hermanos. No se crea, mueve ni borra ningún nodo del plugin, así que es restilar y no rearmar (§6.2). El riel mide 116px; sus filas y las opciones del panel siguen en 48px. Un punto en la categoría marca que hay selección (aplicada o pendiente).
