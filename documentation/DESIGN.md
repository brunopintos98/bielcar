...
Con una excepción, aprobada explícitamente y acotada en CLAUDE.md: el chrome de filtros en mobile **inserta un contenedor propio** dentro de `#FilterContent` (§8). Sigue sin mutarse ningún nodo del plugin, y las condiciones que hacen segura esa inserción son parte de la excepción. No se extiende por analogía a otros casos.
...
**Cuánto cubre `#CurrentFilters` NO está verificado.** No sabemos si el plugin emite un chip por cada filtro aplicado o solo por algunos. Lo único observado: una captura de desktop mostró dos chips, "Autos y Camionetas ✕" y "Nuevos ✕", y ninguno de marca — pero no es la misma URL del caso que motivó la pregunta, así que no alcanza para concluir nada en ninguna dirección. Por eso **nada depende de esos chips para saber qué hay aplicado**: eso sale de las keys `ma_*` de la URL (§8). Cuando están, los chips se usan solo para mostrar un texto más legible que el valor crudo.
...
`ma_status` · `ma_brand` · `ma_model` · `ma_type` · `ma_subtype` · `ma_sort` · `ma_min_price` · `ma_max_price` · `ma_min_km` · `ma_max_km` · `ma_fuel` · `ma_transmission` · `ma_page` · `ma_carid`

**Esta lista es un piso, no un techo.** `ma_model` (label "Modelo") se descubrió tarde, filtrando por marca: no estaba acá. El motivo es estructural y conviene tenerlo presente antes de dar la lista por completa — **las categorías del panel son dinámicas**: el plugin ofrece unas u otras según lo que ya esté filtrado, y "Modelo" no existe hasta que hay una marca puesta (entonces ofrece los modelos de esa marca). Una categoría que solo aparece en el tercer nivel de drill-down puede no haberse visto nunca. Consecuencia práctica: **no escribir mapas `ma_* → "Nombre"` a mano.** Si hace falta el nombre de una categoría, se lee del `.filter-block-label` en el load donde el bloque está presente (§8, badge de filtro aplicado).

Corolario del mismo hecho: los índices posicionales de los bloques del panel se corren entre page loads cuando una categoría aparece o desaparece. Nada persistido puede indexarse por posición; la key `ma_*` es lo único estable.
...
Consecuencia de diseño: el botón "Compartir" que el plugin ya trae en el detalle funciona de verdad. No hace falta construir uno propio, sí restilarlo — pero restilarlo bien requirió capturar el DOM real, porque no es markup del propio Multiaviso: es [ShareThis](https://sharethis.com/), un widget de terceros que Multiaviso embebe (`.sharethis-inline-share-buttons`). Aplica la misma regla del proyecto igual: árbol ajeno, se restila con CSS, nunca se toca con JS.

En mobile, capturado con devtools en producción, los cuatro botones de red (Facebook, X/Twitter, WhatsApp, copiar link) llegan con `style="display: none"` inline puesto por el propio widget — no hay panel de redes propio que restilar en este viewport. El único botón visible es `.st-btn.mobile-share`, que dispara la hoja de compartir **nativa** del sistema operativo (Web Share API), no un menú de ShareThis. El ícono es un `<svg>` inline con `fill="currentColor"`, así que se restila entero con `color`. Ver el detalle completo y las reglas en `src/styles/multiaviso.css`, sección 9 ("detalle del vehículo").
...
- **El chrome es markup nuestro**, en `MultiavisoCatalog.astro`, fuera de `#MultiavisoContainer`. Cerrar despacha un click sobre `#FiltersTitle`. Sobre los nodos del plugin, el único write sigue siendo `data-ma-pending` en la opción elegida (nunca `class`: el observer filtra `style`/`class`). Lo que sí cambió es que **se inserta un contenedor nuestro** dentro de `#FilterContent` —la zona de filtros aplicados, más abajo en esta misma sección—: es la excepción acotada que CLAUDE.md autoriza, y sigue sin mutarse ningún nodo ajeno.
- **Los filtros no se aplican al toque.** Cada opción es un link del plugin que recargaría la página; en mobile interceptamos el click (`preventDefault`, sin mutar el markup) y acumulamos la selección. **"Ver resultados"** navega a la URL compuesta. **✕ / backdrop / Escape** cierran y descartan lo pendiente: los filtros que ya estaban en la URL no se tocan. **"Limpiar filtros"** vacía la selección pendiente y, al confirmar con "Ver resultados", va a la ruta sin `ma_*`. El orden también espera a "Ver resultados".
- **Trade-offs aceptados** (el plugin no reconsulta al elegir): los contadores "(45)" quedan del último page load, y el drill-down Marca → Modelo no aparece hasta la recarga de "Ver resultados". Una versión anterior de esta línea decía que por eso no había aplicar-al-final; el usuario lo revirtió después de ver la recarga-por-toque en producción.
- **Sí hay columna lateral de categorías, pero como índice — no como selector de "un grupo por vez".** Los ocho bloques de filtro quedan apilados y visibles a la vez, en un scroll continuo dentro de `#FilterContent`; el riel (`.ma-sheet__rail`, markup nuestro, se arma leyendo los `.filter-block-label`) es un índice con dos gestos: tocar una categoría scrollea suave hasta su bloque (`scrollTo` sobre `#FilterContent`, respetando `prefers-reduced-motion`), y scrollear a mano actualiza cuál categoría está marcada (scrollspy: listener de `scroll` coalescido con `requestAnimationFrame`, misma técnica que el observer de mutaciones). El scrollspy se suprime mientras dura un scroll programático, para que tocar "Transmisión" no encienda de paso todas las categorías intermedias camino a destino. No se crea, mueve ni borra ningún nodo del plugin — leer `offsetTop`, observar y llamar `scrollTo`/`scrollIntoView` es lectura, no mutación — así que sigue siendo restilar y no rearmar (§6.2). El riel mide 116px; sus filas y las opciones del panel siguen en 48px. (Estuvo en 140px mientras el badge del filtro aplicado vivía adentro del riel; al mudarse a la zona de aplicados de la derecha volvió a 116, que a 13px deja pasar todos los labels salvo "Ordenar vehículos" y los dos "Rango de…", que envuelven a dos líneas.) Un punto en la categoría marca que hay selección, pendiente o aplicada, y se deriva de las keys `ma_*` que ese bloque **realmente controla** — las que varían de valor entre sus opciones, no las que están presentes en todas. La distinción no es teórica: cuando ya hay un filtro en la URL, el contexto viaja en todos los links del bloque (`?ma_status=USED&ma_brand=Honda`, `?ma_status=USED&ma_brand=Geely`), así que "presente en todos" no identifica nada y encendía el punto en todas las categorías a la vez. El label de cada bloque, que antes se ocultaba porque el riel ya decía cuál era la categoría activa, ahora es el encabezado visible de su sección.

  Esto reemplaza un mecanismo anterior de "un grupo por vez": mostrar solo el bloque activo con `display` gateado por `html[data-ma-cat='N'] … :nth-child(N)`, una regla por número y un tope duro de 12 categorías. Se cambió porque, con ocho bloques cortos, no había necesidad real de ocultar contenido — apilarlos y navegar con scroll evita la regla-por-número y el tope, a costa de un panel más largo que ya estaba pensado para scrollear (§8, más arriba).
- **Los filtros aplicados son una zona propia DENTRO del scroll del panel.** `.ma-sheet__applied`: contenedor nuestro que el script inserta adentro de `#FilterContent`, justo después del bloque de orden. Adentro, **apilados en vertical, una sección por categoría aplicada**: el nombre de la categoría como encabezado y debajo su badge con el valor y la ✕.

  ```
  1 a 12 de 12 resultados     ← #Counts, del plugin
  ORDENAR VEHÍCULOS           ← bloque de orden, del plugin
  ────────────────
  MARCA                       ← nuestra zona, insertada acá
  ⌜Geely ✕⌝
  CARROCERÍA
  ⌜Suv ✕⌝
  ────────────────
  CARROCERÍA                  ← siguen los bloques del plugin
  …
  ```

  Todo eso es **un solo scroll**.

  El encabezado **imita** el estilo de `.filter-block-label` del plugin (uppercase, `--fs-eyebrow`, `--tracking-caps`, `--ma-text`) para que la columna se lea continua y no como dos sistemas pegados — pero es markup nuestro, no un nodo del plugin ni una copia de uno. Si de esa categoría no se aprendió el nombre, va el badge **sin encabezado**: nunca uno inventado. El nombre queda escrito dos veces (riel y encabezado) y es una decisión tomada a la vista de la alternativa, que era alinear cada badge con su fila del riel y obligaba a sincronizar alturas entre dos columnas. El valor se muestra con inicial mayúscula (`suv` → `Suv`) por `text-transform: capitalize` sobre el span: presentación pura, el dato y el `href` de remoción siguen crudos.

  El badge entero es el control táctil (44px de alto); la ✕ es la afordancia adentro del target, no un target aparte.

  **No le roba alto a nada.** Al estar en el flujo, crece lo que tenga que crecer y el panel scrollea. Esto reemplaza a una versión anterior en la que la zona era una caja `position: fixed` apoyada arriba del panel: ahí el panel tenía que descontar su alto (`--ma-applied-h`, medido por JS y publicado como custom property) y la zona necesitaba tope y scroll propio, porque con 4 filtros se comía el **50%** de la hoja en un teléfono de 390×844 —el panel quedaba en ~207px— y el **66%** en uno de 360×640, donde bajaba a ~44px. Toda esa maquinaria se eliminó: `--ma-applied-h`, el `ResizeObserver`, la medición y el tope.

  **El precio: esto inserta un nodo nuestro en el árbol del plugin.** Es la excepción acotada que CLAUDE.md autoriza, y las condiciones que la hacen segura están ahí: contenedor propio, nunca un nodo ajeno mutado, **nunca con la clase `.filter-block`** (el script indexa con `:scope > .filter-block` y por posición: un nodo nuestro con esa clase correría el índice de todas las categorías), e inserción idempotente re-verificada en el tick que ya existe — que es además lo que evita el loop con el `MutationObserver`.

  Dos detalles que la inserción vuelve sensibles y conviene no perder de vista. **`:nth-child` cuenta TODOS los hijos**, así que las dos reglas que dependen de la posición —`> .filter-block:nth-child(1)` para `#Counts` y `> .filter-block:nth-child(n+2)` para el filete de encabezado— sobreviven solo porque insertamos DESPUÉS del bloque de orden: `#Counts` sigue siendo el hijo 1 y todos los demás siguen en ≥2. Insertar en otra posición las rompería. Y el scrollspy pasó a ordenar sus secciones por `offsetTop` en vez de por índice de bloque, porque con un nodo nuestro intercalado el orden visual ya no se deduce del índice.

- **El orden de las dos columnas.** La columna derecha, en un solo scroll: `#Counts` → bloque de orden → nuestras secciones de activas → el resto de los bloques. El riel copia ese orden: **"Ordenar vehículos" → activas → resto**. Y las filas de activas **sí son anclaje**: tocarlas scrollea hasta la zona de aplicados, como cualquier otra fila lleva a su bloque.

  Esto se decidió dos veces y conviene saber por qué, para no leerlo como capricho. Primero se descartó insertar un contenedor propio en `#FilterContent` —la única forma de intercalar nuestras secciones entre el bloque de orden y los demás— para respetar la regla de no tocar el árbol del plugin; con la zona como caja fija arriba, el orden visual quedaba activas → orden → resto, el riel copiaba eso, y las filas de activas no podían anclar a ningún lado. Después el usuario vio esa versión funcionando en el teléfono, pidió que la zona scrolleara con la lista, y **se adoptó la inserción** con la excepción acotada en CLAUDE.md. Lo que cambió no fue el criterio sino la información: la segunda decisión se tomó sobre la primera andando.

  **Callejón sin salida, documentado para que nadie lo vuelva a intentar:** poner `#FilterContent` en `display: flex` + `order` sobre los hijos para reordenar sin mover nada de lugar **no funciona**. jQuery abre el panel escribiendo `display: block` INLINE, que le gana a la hoja: el panel nunca sería flex y `order` no haría nada. Forzarlo con `display: flex !important` está permitido por §6.3 para el caso de estilos inline, pero entonces el `display: none` con el que jQuery cierra también quedaría pisado y `isOpen()` —que decide el estado leyendo `getComputedStyle(panel).display`— nunca leería el panel como cerrado: se rompe la máquina de apertura entera. El orden correcto se logra insertando en la posición correcta, no reordenando.
