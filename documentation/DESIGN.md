...
- **Cada filtro es un link que recarga la página**, así que no hay "aplicar al final", y no es una decisión de diseño: la taxonomía del plugin es **progresiva** (el grupo "Marca" se reemplaza por "Modelo" recién cuando hay una marca aplicada) y los contadores se recalculan contra el set aplicado ("SUV (45)" pasa a "SUV (9)" con Honda). Acumular sin recargar dejaría los números mintiendo y haría inalcanzable el drill-down a modelo. Verificado en producción. En su lugar, **la hoja se reabre en la misma categoría después de la recarga**: el label activo se guarda en `sessionStorage` antes de navegar y al cargar se despacha el click en `#FiltersTitle`. Si el label ya no existe (p. ej. Marca → Modelo), cae al primero.
- **Sí hay columna lateral de categorías**, como en ML. Una versión anterior de esta línea decía que no, porque "exigiría rearmar el DOM del plugin": era incorrecto. El riel es markup nuestro (`.ma-sheet__rail`) y se arma leyendo los `.filter-block-label`; mostrar un grupo por vez es `display` por `:nth-child(N)` sobre los hijos de `#FilterContent`, que son todos `.filter-block` hermanos. No se crea, mueve ni borra ningún nodo del plugin, así que es restilar y no rearmar (§6.2). El riel mide 116px; sus filas y las opciones del panel siguen en 48px. El pie lleva **"Limpiar filtros"** (link a la ruta sin querystring, solo si hay `ma_*` aplicados) y **"Ver resultados"** (cierra la hoja).
