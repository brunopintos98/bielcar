...
- **El chrome es markup nuestro**, en `MultiavisoCatalog.astro`, fuera de `#MultiavisoContainer`. Cerrar despacha un click sobre `#FiltersTitle`. El único write al árbol del plugin es `data-ma-pending` en la opción elegida (nunca `class`: el observer filtra `style`/`class`).
